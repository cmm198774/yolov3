...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            #model_type=module_def["type"]
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                x = module(x)
                #logger.debug(f"This is level {i},level type ={model_type},shape of output={str(x.shape)},output device={str(x.device)}")
            elif module_def["type"] == "route":
                x = torch.cat([layer_outputs[int(layer_i)] for layer_i in module_def["layers"].split(",")], 1)
                layer_info=module_def["layers"]
                #logger.debug(f"This is level {i},level type ={model_type},input_layer={layer_info},shape of output={str(x.shape)},output device={str(x.device)}")
            elif module_def["type"] == "shortcut":
                layer_i = int(module_def["from"])
                x = layer_outputs[-1] + layer_outputs[layer_i]
                #logger.debug(f"This is level {i},level type ={model_type},input_layer={layer_i},shape of output={str(x.shape)},output device={str(x.device)}")
            elif module_def["type"] == "yolo":
